...
flowchart TD
    ...
    C --> C7[Hours Calculator<br/>Local complete]
...
| P1 | Hours Calculator | 時數計算 | 時數／時間差計算 | Low — **local complete（standalone＋Link Integration；尚未 push／deploy）** |
...
9. Hours Calculator — local complete（standalone＋Link Integration；尚未 push／deploy）
