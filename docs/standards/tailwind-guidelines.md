...
## 5.1 Global Interactive Cursor Baseline

Timiva 在 `src/styles/global.css` `@layer base` 為 enabled 語意化控制項恢復 `cursor: pointer`。

```text
Timiva restores pointer cursor globally for enabled semantic controls.

Do not add cursor:pointer or cursor-pointer locally to ordinary buttons,
links, summaries, selects, or interactive roles.

Use cursor utilities only for exceptional cursor behavior,
such as grab, resizing, text entry, progress, not-allowed, or custom states.

Disabled and aria-disabled controls must not be overridden by later local pointer rules.
```
