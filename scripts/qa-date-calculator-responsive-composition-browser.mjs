/**
 * Date Calculator — Responsive Composition Browser QA (Batch 3B).
 *
 * Run after build: npx astro preview --port 4360 &
 *   DCV2_QA_BASE=http://localhost:4360 node scripts/qa-date-calculator-responsive-composition-browser.mjs
 */
import { mkdirSync, writeFileSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";

const BASE = process.env.DCV2_QA_BASE ?? process.env.LDC_QA_BASE ?? "http://localhost:4360";
const rootDir = join(dirname(fileURLToPath(import.meta.url)), "..");
const evidencePath = join(
	rootDir,
	"local-docs/validation/date-calculator-responsive-composition-batch3b-evidence.json",
);
...
async function openPage(browser, path, viewport, hoverMode) {
	...
	await page.waitForSelector("[data-date-calculator-v2]");
	return { context, page };
}
...
async function measureDcComposition(page) {
	return page.evaluate(() => {
		...
		const mobile = document.querySelector(
			"[data-date-calculator-v2] .dcv2-mobile-controls",
		);
		const capsule = document.querySelector(
			"[data-date-calculator-v2] .dcv2-mobile-capsule",
		);
...
async function measureAmeShell(page) {
	return page.evaluate(() => {
		const root = document.querySelector("[data-date-calculator-v2] [data-ame-root]");
...
console.log("qa-date-calculator-responsive-composition-browser\n");
console.log(`base: ${BASE}\n`);

mkdirSync(join(rootDir, "local-docs/validation"), { recursive: true });
await waitForServer(`${BASE}/en/date-calculator/`);

const pw = await loadPlaywright();
const browser = await pw.chromium.launch({ headless: true });
const enPath = "/en/date-calculator/";
const zhPath = "/zh/date-calculator/";
...
for (const localePath of [enPath, zhPath]) {
	...
	const ops = await measureDcComposition(page);
	note(
		`${locale} 700×500: rs=${ops.rsLayout} mode=${ops.resolveMode} compact=${ops.landscapeCompact} maxW=${ops.capsuleMaxWidth}`,
	);
...
for (const { viewport, hover, label, expectFs } of [
	...
	await page.locator("[data-dcv2-sheet-trigger]").click();
	await page.waitForFunction(
		() =>
			document
				.querySelector("[data-date-calculator-v2] [data-ame-root]")
...
console.log("qa-date-calculator-responsive-composition-browser PASS");
