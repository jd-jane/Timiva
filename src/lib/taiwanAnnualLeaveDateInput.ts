/**
 * Taiwan Annual Leave — tool-local Smart Date Input (B2B Desktop).
 * Segment engine follows docs/standards/date-input.md (Age / DBD patterns).
 * Hire-date bounds: 1900-01-01 … today（含）；future = invalid.
 * Not a shared module; does not fork DesktopCalendar.
 */
...
/** Year-list upper bound helper；實際可選日以 today 為準（見 isDateInAllowedRange）。 */
...
export function applySegmentInputChange(
	...
	if (inputType === "insertText" && data && /^\d$/.test(data)) {
		...
		/* Continuous raw stream：即使 beforeinput 回報 stale caret，仍必須整串 re-derive.
		 * Segment edit 只在 preferStream === false 時依 caret 進行（BDC canonical）. */
...
/** Hire date：≥ 1900-01-01 且 ≤ today（含）。 */
export function isDateInAllowedRange(
	date: CalendarDate,
	...
	const min = { year: MIN_DATE_YEAR, month: 1, day: 1 };
	if (compareCalendarDates(date, min) < 0) {
		return false;
	}
...
export function isSelectableHireDate(
	date: CalendarDate,
	today: CalendarDate = getTodayCalendarDate(),
): boolean {
	return isDateInAllowedRange(date, today);
}
...
export function parseDateSegments(segments: DateSegments): CalendarDate | null {
	...
	if (!isDateInAllowedRange(date, getTodayCalendarDate())) {
		return null;
	}
